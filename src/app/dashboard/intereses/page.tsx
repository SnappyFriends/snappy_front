...
export default function InterestsDashboard() {
  ...
  const [editingInterest, setEditingInterest] = useState<IInterest | null>(null);
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/interests/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        throw new Error("Error deleting interest");
      }
      setInterests(interests.filter((interest) => interest.interest_id !== id));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (editingInterest) {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/interests/${editingInterest.interest_id}`, {
          method: "PUT",
  ...
  return (
    <div className="pb-20 flex flex-col gap-6">
      <h1 className="text-2xl text-center font-bold mb-6">Gestionar Intereses</h1>
        ...
        <tbody>
          {interests.map((interest) => (
            <tr key={interest.interest_id} className="text-center">
              ...
              <td className="border border-gray-300 p-4">{interest.interest_id}</td>
              <td className="border border-gray-300 p-4 flex justify-center gap-4">
              <button
      ...
      {editingInterest && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center">
          ...
          >
            ...
            <label className="block mb-2 text-sm font-medium">Nuevo Nombre</label>
            <input
              placeholder="Nuevo nombre"
              type="text"
              value={newInterest}
